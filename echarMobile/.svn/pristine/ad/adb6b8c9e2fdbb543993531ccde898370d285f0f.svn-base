$(function(){
 var myChart = echarts.init(document.getElementById('learnActivities'));
//myChart.showLoading();
...
var diskData = [
    ...
    for(var i in diskData){
    	var currOBJ = diskData[i]
    	var currCount = 0;
    	if(diskData[i].children !==undefined){
    		for(var j in diskData[i].children){
    			var currChild = diskData[i].children[j];
    			var currNum = randomData(1000);
    			currCount += currNum;
    			diskData[i].children[j].value = currNum;
    		}    		
    	}
    	else{
    		currCount += randomData(1000);
    	}
    	diskData[i].value = currCount;
    }
        ...
        tooltip: {
            formatter: function (info) {
                ...
                return [
                    ...
                    '参与学习: ' + formatUtil.addCommas(value)+'人' ,
                ].join('');
            }
        },
